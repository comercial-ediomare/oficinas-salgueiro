...
def inscrever():
    ...
    if not consent:
        flash("Você precisa concordar com o tratamento dos seus dados.", "error"); return redirect(url_for("index"))
    if not full_name:
        flash("Informe seu nome completo.", "error"); return redirect(url_for("index"))
    if not email or "@" not in email:
        flash("Informe um e-mail válido.", "error"); return redirect(url_for("index"))
    if len(chosen_ids) < 1:
        flash("Selecione pelo menos 1 oficina (uma por horário).", "error"); return redirect(url_for("index"))
    if len(chosen_ids) > 4:
        flash("Você pode selecionar no máximo 4 oficinas (uma por horário).", "error"); return redirect(url_for("index"))
    if len(chosen_ids) != len(set(chosen_ids)):
        flash("Você não pode escolher a mesma oficina em horários diferentes.", "error"); return redirect(url_for("index"))
    ...
    try:
        ...
        if cur.fetchone():
            conn.execute("ROLLBACK"); flash("Este e-mail já está inscrito.", "error"); return redirect(url_for("index"))

        for wid in chosen_ids:
            cur.execute("SELECT capacity, registered FROM workshops WHERE id = ?", (wid,))
            row = cur.fetchone()
            if not row:
                conn.execute("ROLLBACK"); flash("Oficina inválida.", "error"); return redirect(url_for("index"))
            cap, reg = row["capacity"], row["registered"]
            if reg >= cap:
                conn.execute("ROLLBACK"); flash("Uma das oficinas esgotou enquanto você enviava. Selecione outra.", "error"); return redirect(url_for("index"))
            cur.execute("UPDATE workshops SET registered = registered + 1 WHERE id = ?", (wid,))

        now = datetime.datetime.utcnow().isoformat()
        cur.execute("INSERT INTO attendees(full_name, email, selections, created_at) VALUES (?, ?, ?, ?)",
...
def admin():
    ...
    parsed = []
    for a in attendees:
        try: sels = json.loads(a["selections"]) or []
...
def export_csv():
    ...
    writer.writerow(["WORKSHOPS"]); writer.writerow(["id","name","capacity","registered","remaining"])
    for w in ws: writer.writerow([w["id"], w["name"], w["capacity"], w["registered"], w["capacity"]-w["registered"]])
    writer.writerow([]); writer.writerow(["ATTENDEES"]); writer.writerow(["full_name","email","selections_ids","created_at"])
    for a in at: writer.writerow([a["full_name"], a["email"], a["selections"], a["created_at"]])
    mem = io.BytesIO(output.getvalue().encode("utf-8")); mem.seek(0)
    resp = make_response(mem.read()); resp.headers.set("Content-Type","text/csv; charset=utf-8")
