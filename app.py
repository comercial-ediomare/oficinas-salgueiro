...
# Em produção, prefira montar um Disk no Render em /data e apontar DB_PATH para /data/inscricoes.db
...
def init_db():
    with closing(get_db()) as conn:
        ...
        """)
        # --- (A) MIGRAÇÃO LEVE: garantir coluna selections_map (JSON {slot_id: workshop_id}) ---
...
def inscrever():
    ...
    try:
        ...
        # --- (B) Registra participante salvando lista e mapa por horário ---
...
def build_reports():
    """
    Retorna:
      - by_workshop: {wid: {"name": str, "count": int}}
      - by_slot: {slot_id: {"hora": str, "items": {wid: {"name": str, "count": int}}}}
    """
    with closing(get_db()) as conn:
        id2name = _workshop_map(conn)
        attendees = conn.execute(
            "SELECT selections, selections_map FROM attendees"
        ).fetchall()

    # por oficina (independe de horário)
    ...
    by_slot = {
        s["id"]: {
            "hora": s["hora"],
            "items": {wid: {"name": id2name[wid], "count": 0} for wid in id2name}
        } for s in SLOTS
    }

    for a in attendees:
        # Contagem por oficina (usa 'selections' lista de IDs, inclusive legado)
        try:
            sel_list = json.loads(a["selections"]) if a["selections"] else []
            if isinstance(sel_list, list):
                for wid in sel_list:
                    if wid in by_workshop:
                        by_workshop[wid]["count"] += 1
        except Exception:
            pass

        # Contagem por horário (usa 'selections_map' {slot_id: wid})
        try:
            sel_map = json.loads(a["selections_map"]) if a["selections_map"] else {}
            if isinstance(sel_map, dict):
                for sid_str, wid in sel_map.items():
                    try:
                        sid = int(sid_str)
                    except Exception:
                        sid = sid_str
                    if sid in by_slot and wid in by_slot[sid]["items"]:
...
# --- (D) Rotas de relatórios (view + exports) ---
@app.route("/reports")
@login_required
def reports():
    by_workshop, by_slot = build_reports()
    # Ordena por nome de oficina
    ws_sorted = sorted(by_workshop.items(), key=lambda kv: kv[1]["name"])
    # Prepara estrutura amigável para o template
    slots_view = []
    for s in SLOTS:
        sid = s["id"]
...
def export_by_workshop_csv():
    ...
    resp = make_response(mem.read())
    resp.headers.set("Content-Type", "text/csv; charset=utf-8")
    resp.headers.set("Content-Disposition", "attachment", filename="relatorio_por_oficina.csv")
    return resp
...
def export_by_slot_csv():
    ...
    resp = make_response(mem.read())
    resp.headers.set("Content-Type", "text/csv; charset=utf-8")
    resp.headers.set("Content-Disposition", "attachment", filename="relatorio_por_horario.csv")
    return resp
